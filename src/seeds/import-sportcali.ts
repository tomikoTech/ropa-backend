/**
 * Importador de migración: Sportcali (demachine) -> MiPinta.
 *
 * Lee migracion-sportcali/out/payload.json (generado por extract.py) y crea
 * de forma IDEMPOTENTE en MiPinta:
 *   - Tenant "sportcali" + StoreSettings
 *   - 5 bodegas (Warehouse)
 *   - Staff (User) a partir de los usuarios de demachine
 *   - Productos (con columna `brand`), variantes (talla/color) y stock por bodega
 *
 * Idempotencia: productos por `sourceRef` = "demachine:sportcali:<id>";
...
const DEFAULT_STAFF_PASSWORD = 'sportcali123'; // temporal; que la cambien al entrar
...
async function main() {
  ...
  for (const s of payload.staff) {
    ...
    if (!user) {
      ...
      // Cesar es el dueño de la tienda -> ADMIN (necesario para gestionar el tenant).
          ...
          if (qty > 0) {
            await m.getRepository(StockMovement).save(
              m.getRepository(StockMovement).create({
                ...
                quantity: qty,
                referenceType: 'IMPORT_SPORTCALI',
