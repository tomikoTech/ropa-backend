...
export class StoreSettingsService {
  ...
  async updateSettings(tenantId: string, dto: UpdateStoreSettingsDto): Promise<StoreSettings> {
    ...
    if (dto.isStorefrontActive !== undefined) settings.isStorefrontActive = dto.isStorefrontActive;
    if (dto.defaultWarehouseId !== undefined) settings.defaultWarehouseId = dto.defaultWarehouseId;
  ...
  async updateOrderStatus(
    ...
    if (order.status === EcommerceOrderStatus.DELIVERED) {
      throw new BadRequestException('No se puede actualizar un pedido entregado');
    }
  ...
  async finalizeOrder(id: string, userId: string, tenantId: string): Promise<EcommerceOrder> {
    return this.dataSource.transaction(async (manager) => {
      ...
      for (const item of order.items) {
        const stock = await stockRepo.findOne({
          where: { variantId: item.variantId, warehouseId: order.warehouseId, tenantId },
        });
  ...
  async cancelOrder(id: string, _userId: string, tenantId: string): Promise<EcommerceOrder> {
    const order = await this.orderRepo.findOne({
      where: { id, tenantId },
      relations: ['items'],
    });
